...
def carregar_post_it():
    ...
    return ["Silvio Santos", "Harry Potter", "Uma Geladeira", "Um Papagaio", "Homem-Aranha", "Monalisa", "Albert Einstein", "Um Micro-ondas", "Cleópatra", "Batman", "Um Abacaxi", "Michael Jackson", "Uma Árvore", "Super Mario", "Um Celular"]
...
def carregar_verdade_desafio():
    ...
    return {
        "VERDADE": ["Qual foi a maior mentira que você já contou?", "De quem você tem mais ciúmes?", "Qual foi o seu momento mais constrangedor?"],
        "DESAFIO": ["Imite um animal escolhido pelo grupo por 1 minuto.", "Deixe a pessoa à sua direita enviar uma mensagem do seu celular.", "Dance sem música por 30 segundos."]
    }
...
def app():
    ...
    # Menu Principal em 2 linhas
    col1, col2, col3 = st.columns(3)
    with col1:
        if st.button("🤥 2 Verdades 1 Mentira", use_container_width=True): iniciar_duas_verdades()
        if st.button("🏷️ Post-it na Testa", use_container_width=True): st.session_state.jogo_interacao_ativo = "post_it"
    with col2:
        if st.button("🫣 Eu Nunca", use_container_width=True): iniciar_eu_nunca()
        if st.button("🎲 Verdade ou Desafio", use_container_width=True): st.session_state.jogo_interacao_ativo = "truth_or_dare"
    with col3:
        if st.button("❓ 20 Perguntas", use_container_width=True): iniciar_vinte_perguntas()
    ...
    if st.session_state.jogo_interacao_ativo == "duas_verdades":
        ...
        if st.session_state.dv_fase == "preencher":
            st.info("Preencha duas coisas verdadeiras sobre você e uma falsa. O app vai embaralhá-las!")
            ...
            if st.button("Embaralhar e Desafiar o Grupo", type="primary"):
                if v1 and v2 and m1:
                    afirmacoes = [{"texto": v1, "tipo": "Verdade", "cor": "green"}, {"texto": v2, "tipo": "Verdade", "cor": "green"}, {"texto": m1, "tipo": "Mentira", "cor": "red"}]
            ...
            for i, af in enumerate(st.session_state.dv_afirmacoes):
                with st.expander(f"Opção {i+1}: {af['texto']}"):
                    st.markdown(f"<h3 style='color: {af['cor']};'>{af['tipo']}!</h3>", unsafe_allow_html=True)
    ...
    # --- LÓGICA: VINTE PERGUNTAS ---
        ...
        st.markdown(f"<h2 style='text-align: center;'>{st.session_state.vp_contador} / 20</h2>", unsafe_allow_html=True)
        
        col_a, col_b, col_c = st.columns([1, 2, 1])
        with col_a:
            if st.button("➖ Desfazer", use_container_width=True, disabled=(st.session_state.vp_contador <= 0)):
                st.session_state.vp_contador -= 1
                st.rerun()
        with col_b:
            if st.button("➕ Fizeram Pergunta", type="primary", use_container_width=True, disabled=(st.session_state.vp_contador >= 20)):
                st.session_state.vp_contador += 1
                st.rerun()
        with col_c:
