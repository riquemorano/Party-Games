...
def carregar_palavras_spy():
    ...
    # Fallback de pares semelhantes para o jogo Spy
    return [
        ("Maçã", "Pera"), ("Carro", "Caminhão"), ("Cachorro", "Lobo"), 
        ("Futebol", "Basquete"), ("Praia", "Piscina"), ("Cinema", "Teatro"),
        ("Rei", "Presidente"), ("Violão", "Guitarra"), ("Ouro", "Prata")
    ]
...
def distribuir_papeis_resistencia(jogadores):
    ...
    papeis = ["🕵️ Espião (Traidor)"] * num_espioes + ["✊ Operativo (Resistência)"] * num_resistencia
    random.shuffle(papeis)
    return dict(zip(jogadores, papeis)), []

def distribuir_papeis_lobisomem(jogadores):
    num_jogadores = len(jogadores)
    papeis_disponiveis = ["🐺 Lobisomem", "🐺 Lobisomem", "👁️ Vidente", "🦹 Ladrão", "🔄 Encrenqueiro"]
...
def app():
    ...
    if st.session_state.jogo_social_ativo in ["resistencia", "lobisomem"]:
        nome_jogo = "A Resistência" if st.session_state.jogo_social_ativo == "resistencia" else "Lobisomem de Uma Noite"
        ...
        if not st.session_state.jogo_iniciado:
            ...
            if st.session_state.jogadores_social:
                st.write("**Jogadores na partida:**", ", ".join(st.session_state.jogadores_social))

            min_jogadores = 5 if st.session_state.jogo_social_ativo == "resistencia" else 3
            if len(st.session_state.jogadores_social) >= min_jogadores:
                if st.button("🚀 Iniciar Jogo e Sortear Papéis", use_container_width=True, type="primary"):
                    if st.session_state.jogo_social_ativo == "resistencia":
                        papeis, centro = distribuir_papeis_resistencia(st.session_state.jogadores_social)
                    else:
                        papeis, centro = distribuir_papeis_lobisomem(st.session_state.jogadores_social)
                ...
                st.info(f"Adicione pelo menos {min_jogadores} jogadores para jogar esta variante.")
        else:
            st.success("Papéis distribuídos! Passem o dispositivo para que cada um veja seu papel em segredo.")
            for jogador in st.session_state.jogadores_social:
                with st.expander(f"Visualizar papel de: {jogador}"):
                    if st.button(f"👁️ Revelar meu papel ({jogador})", key=f"btn_{jogador}"):
                    ...
                    if st.session_state.visualizando_jogador == jogador:
                        ...
                        # Dicas extras para espiões/lobisomens se reconhecerem
                        if papel == "🕵️ Espião (Traidor)":
                            outros_espioes = [j for j, p in st.session_state.papeis_distribuidos.items() if p == "🕵️ Espião (Traidor)" and j != jogador]
                            if outros_espioes:
                                st.error(f"Seus parceiros espiões são: {', '.join(outros_espioes)}")
                                
                        elif papel == "🐺 Lobisomem":
                            outros_lobos = [j for j, p in st.session_state.papeis_distribuidos.items() if p == "🐺 Lobisomem" and j != jogador]
                            if outros_lobos:
                                st.error(f"Seu parceiro lobisomem é: {', '.join(outros_lobos)}")
                        
                        if st.button("🙈 Esconder e Passar a Vez", key=f"hide_{jogador}"):
                            st.session_state.visualizando_jogador = None
                            st.rerun()

            st.divider()
            if st.session_state.jogo_social_ativo == "lobisomem" and st.session_state.cartas_centro:
                with st.expander("👑 Ver Cartas do Centro (Apenas Narrador/Vidente)"):
                    st.write(f"As cartas no centro são: **{', '.join(st.session_state.cartas_centro)}**")
        ...
        if not st.session_state.mafia_game["iniciado"]:
            ...
            if n_extras > 0:
                ...
                for i in range(n_extras):
                    ...
                    nome_papel = c1.text_input(f"Nome do Papel {i+1}", f"Papel Extra {i+1}", key=f"name_ex_{i}")
                    qtd_papel = c2.number_input(f"Qtd", 0, n_jogadores, 1, key=f"qtd_ex_{i}")
                    papeis_extras_dict[nome_papel] = qtd_papel

            total_especiais = n_assassinos + n_detetive + n_medico + sum(papeis_extras_dict.values())
            n_cidadaos = n_jogadores - total_especiais

            st.divider()
            if n_cidadaos < 0:
                st.error(f"❌ **Erro!** Você distribuiu {total_especiais} papéis para apenas {n_jogadores} jogadores. Remova {abs(n_cidadaos)} papel(éis).")
                btn_ready = False
            else:
                st.info(f"✅ **Pronto!** {total_especiais} papéis especiais e {n_cidadaos} Cidadãos Comuns.")
                btn_ready = True
            ...
            if st.button("Sortear e Começar", disabled=not btn_ready, use_container_width=True, type="primary"):
                ...
                st.session_state.mafia_game.update({"iniciado": True, "papeis": baralho, "atual": 0, "revelado": False})
                st.rerun()
            ...
            if atual < total:
                ...
                if not st.session_state.mafia_game["revelado"]:
                    ...
                    if st.button("👁️ Clique para ver seu papel", key=f"btn_rev_{atual}", use_container_width=True):
                    ...
                    if st.button("🙈 Entendido! Esconder e Passar", key=f"btn_next_{atual}", use_container_width=True):
                ...
                if st.button("🔄 Reiniciar Jogo", use_container_width=True):
                    st.session_state.mafia_game = {"iniciado": False, "papeis": [], "atual": 0, "revelado": False}
                    st.rerun()
        ...
        if not st.session_state.spy_game["iniciado"]:
            ...
            if atual < total_spy:
                ...
                if not st.session_state.spy_game["revelado"]:
                    ...
                    if st.button(f"👁️ Ver Minha Palavra", key=f"spy_btn_{atual}", use_container_width=True):
                        st.session_state.spy_game["revelado"] = True
                        st.rerun()
                else:
                    st.success(f"Sua palavra é: **{st.session_state.spy_game['palavras'][atual]}**")
                    if st.button("🙈 OK, Esconder e Passar!", key=f"spy_next_{atual}", use_container_width=True):
                ...
                if st.button("🔄 Novo Jogo", use_container_width=True):
                    st.session_state.spy_game.update({"iniciado": False, "palavras": [], "atual": 0, "revelado": False})
                    st.rerun()
